...
ngx_event_accept(ngx_event_t *ev)
{
    ...
    do {
...
#else
        s = accept(lc->fd, (struct sockaddr *) sa, &socklen);//在这里建立连接
#endif

        if (s == (ngx_socket_t) -1) {//建立连接失败
...
#if (NGX_STAT_STUB)
        (void) ngx_atomic_fetch_add(ngx_stat_accepted, 1);//增加接受连接的数量?
#endif

        ngx_accept_disabled = ngx_cycle->connection_n / 8
                              - ngx_cycle->free_connection_n;

        c = ngx_get_connection(s, ev->log);//获取连接对象
        ...
        c->type = SOCK_STREAM;//类型为SOCK_STREAM，也可以是UDP???

#if (NGX_STAT_STUB)
        (void) ngx_atomic_fetch_add(ngx_stat_active, 1);
#endif

        c->pool = ngx_create_pool(ls->pool_size, ev->log);//这里应该是每个连接一个池,内存池?
        if (c->pool == NULL) {
        ...
        *log = ls->log;//这里保存为ls的log
        ...
        if (ls->addr_ntop) {//net to presentation，这里就是进行了封装
...
{
    if (ngx_shmtx_trylock(&ngx_accept_mutex)) {//如果锁定成功

        ngx_log_debug0(NGX_LOG_DEBUG_EVENT, cycle->log, 0,
                       "accept mutex locked");

        if (ngx_accept_mutex_held && ngx_accept_events == 0) {//表示已经设置监听成功了
            return NGX_OK;
        }
