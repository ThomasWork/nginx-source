...
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    hc = ngx_pcalloc(c->pool, sizeof(ngx_http_connection_t));//使用内存池分配空间，使用的是connection的内存池
    if (hc == NULL) {
        ngx_http_close_connection(c);
        return;
    }

    c->data = hc; //设置连接中的data为http连接
    ...
    rev = c->read;  //设置为可读事件
    rev->handler = ngx_http_wait_request_handler;//设置读事件的回调函数
    c->write->handler = ngx_http_empty_handler;//设置写事件的回调函数，在http连接刚建立起来的时候，
    //写事件应该是空的，应该在接收到请求之后再写内容???
    ...
    ngx_add_timer(rev, c->listening->post_accept_timeout);//添加定时器，监控接收请求是否超时
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    n = c->recv(c, b->last, size);//接收字符流

    if (n == NGX_AGAIN) {//这里应该表示被信号等中断?
...
//这里应该表示读取结束
    ...
    c->data = ngx_http_create_request(c);//构建一个新的http请求，并初始化
    if (c->data == NULL) {
        ngx_http_close_connection(c);
        return;
    }

    rev->handler = ngx_http_process_request_line;//所以这里只是添加了事件，但是实际上没有处理?
    ngx_http_process_request_line(rev);// 格式为 GET /uri HTTP/1.1，长度不定，与URI 有关，可能会被调用多次
}

//这个函数应该是用来创建http请求并初始化
ngx_http_request_t *
ngx_http_create_request(ngx_connection_t *c)
{
    ...
    r = ngx_pcalloc(pool, sizeof(ngx_http_request_t));/对http请求分配空间
    if (r == NULL) {
        ngx_destroy_pool(pool);
        return NULL;
    }

    r->pool = pool;//http请求也有一个内存池
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            rev->handler = ngx_http_process_request_headers;//这里将其设置为解析HTTP头部
...
//解析http 的头部，在接收较大的HTTP 头部时，它可能被调用多次
