...
ngx_init_cycle(ngx_cycle_t *old_cycle)
{
    ...
    log = old_cycle->log;//为什么使用old_cycle
    ...
    cycle = ngx_pcalloc(pool, sizeof(ngx_cycle_t));//分配空间并初始化为0
    if (cycle == NULL) {
    ...
    cycle->conf_prefix.data = ngx_pstrdup(pool, &old_cycle->conf_prefix);//使用内存池分配空间并把内容拷贝过去
    ...
    ngx_cpystrn(cycle->conf_file.data, old_cycle->conf_file.data,
                old_cycle->conf_file.len + 1);//这里为什么要加1呢?
    ...
    n = old_cycle->paths.nelts ? old_cycle->paths.nelts : 10;//需要分配多少空间

    cycle->paths.elts = ngx_pcalloc(pool, n * sizeof(ngx_path_t *));//分配空间并清空
